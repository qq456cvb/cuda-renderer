...
__host__ __device__ Ray::Ray() :
  o(Vector()), d(Vector()), t_min(0), t_max(INFINITY), depth(0) {

}

__host__ __device__ Ray::Ray(const Vector &origin, const Vector &dir,
...
__host__ __device__ Vector Ray::operator()(float t) const {
  return o + t * d;
}