...
__host__ __device__ BBox::BBox(const Vector &p)
  : p_min(p), p_max(p)
{

}

__host__ __device__ BBox::BBox(const Vector &p1, const Vector &p2) {
  p_min = Vector(fminf(p1.x, p2.x), fminf(p1.y, p2.y), fminf(p1.z, p2.z));
  p_max = Vector(fmaxf(p1.x, p2.x), fmaxf(p1.y, p2.y), fmaxf(p1.z, p2.z));
}
...
__host__ __device__ bool BBox::inside(const Vector &p) const {
...
__host__ __device__ const Vector& BBox::operator[](int i) const {
  if (i == 0)
  {
    return p_min;
  }
